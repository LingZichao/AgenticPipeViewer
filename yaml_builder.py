...
class Task:
    ...
    resolved_capture: List[str] = field(default_factory=list)

    @classmethod
    def from_dict(cls, data: dict[str, Any], global_scope: str, yaml_builder: 'YamlBuilder') -> 'Task':
        """Create Task from dictionary with resolved signals"""
        task_scope = data.get('scope', '')
        capture = data.get('capture', [])

        # Resolve capture signals
        resolved = yaml_builder.resolve_capture_signals(capture, task_scope, global_scope)
...
class YamlBuilder:
    ...
    def _validate_config(self, config: dict[str, Any], config_path: str) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"[ERROR] Failed to create output directory {self.output_dir}: {e}")
    ...
    def resolve_signals(self, available_signals: List[str], config: dict[str, Any]) -> dict[str, Any]:
        """Resolve signal references and validate against available signals"""
        self.available_signals = available_signals
        self.config = config

        # Resolve capture signals and analyze dependencies for all tasks
        ...
        for task in config.get('tasks', []):
            ...
            # Resolve capture signals
            resolved = self.resolve_capture_signals(capture, task_scope, global_scope)
            task['resolved_capture'] = resolved

            # Analyze condition for $dep references
            task['has_dep_in_condition'] = '$dep.' in condition

            # Analyze capture for {var} pattern
            task['has_pattern_in_capture'] = any('{' in str(sig) and '}' in str(sig) for sig in capture)
        ...
        for task in tasks:
            ...
            for sig in task.get('capture', []):
                if isinstance(sig, str):
                    resolved = self._resolve_signal_path(sig, task_scope, global_scope)
                    all_signals.add(resolved)
            # Collect from condition
            condition = task.get('condition')
            if condition:
                self._collect_signals_from_condition(condition, task_scope, global_scope, all_signals)

        return list(all_signals)

    def _resolve_signal_path(self, signal: str, task_scope: str, global_scope: str) -> str:
    ...
    def _collect_signals_from_condition(self, condition: Union[str, list[str]], task_scope: str,
                                       global_scope: str, signals: set[str]) -> None:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Name):
                    ...
                    if token not in ['True', 'False', 'None']:
                        try:
                            resolved = self._resolve_signal_path(token, task_scope, global_scope)
            ...
            for idx, sig in enumerate(capture):
                if isinstance(sig, str):
                    if not sig or sig.isspace():
                        raise ValueError(f"[ERROR] Task '{task_identifier}' capture[{idx}] signal is empty{line_info}")
    ...
    def build_exec_order(self) -> List[int]:
        ...
        if dep_graph_file and self.output_dir:
            self.export_dependency_graph(dep_graph_file, self.output_dir, result)

        return result

    def export_dependency_graph(self, output_file: str, output_dir: Path, task_execution_order: List[int]) -> None:
    ...
    def format_log_message(self, log_format: str, row_data: dict[str, Any], capture_signals: list[str], row_idx: int) -> str:
    ...
    def resolve_capture_signals(self, capture_signals: List[Any], task_scope: str, global_scope: str) -> list[str]:
        ...
        for sig in capture_signals:
            if isinstance(sig, str):
                ...
                if '{' in sig and '}' in sig:
                    ...
                    resolved_sig = self._resolve_signal_path(sig, task_scope, global_scope)
                    resolved_signals.append(resolved_sig)
            else:
                resolved_signals.append(str(sig))
        return resolved_signals

    def resolve_dep_references(self, value: Any, task_id: str, task_data: dict[str, Any]) -> Any:
        ...
        if isinstance(value, str):
            ...
            return [self.resolve_dep_references(item, task_id, task_data) for item in value]
        elif isinstance(value, dict):
            return {k: self.resolve_dep_references(v, task_id, task_data) for k, v in value.items()}
        else:
            return value
